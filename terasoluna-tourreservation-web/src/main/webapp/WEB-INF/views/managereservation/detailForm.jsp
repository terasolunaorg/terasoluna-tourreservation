  ...
  <div>
    ...
    <div>
      ...
      <div class="span-8 last">
        <table>
          <tr>
            <td><form:form action="${pageContext.request.contextPath}/managereservation/list"
                method="GET">
            ...
            <c:if test="${!f:h(output.limitExceeding)}">
              <c:if test="${f:h(output.reserve.transfer) == 0}">
                <td><form:form
                    action="${pageContext.request.contextPath}/managereservation/update/${f:h(output.reserve.reserveNo)}"
                    METHOD="GET">
                    <spring:message code="label.tr.managereservation.changeReservationBtnMessage"
                      var="changeReservation" />
                    <input id="changeBtn" type="submit" name="form" value="${changeReservation}"
                      tabindex="3" />
                  </form:form></td>
                <td><form:form
                    ...
                    <input id="cancelBtn" type="submit" name="confirm" value="${cancelReservation}"
                      tabindex="3" />
                  </form:form></td>
              </c:if>
            </c:if>
