			...
			<c:forEach items="${rows}" var="row" varStatus="rowStatus">
				<tr>
					...
					<td>
						...
						<fmt:formatNumber value="${row.reserve.adultCount}" pattern="${personCountPattern}" />
						<br />
						<spring:message code="label.tr.common.child" />
						<fmt:formatNumber value="${row.reserve.childCount}" pattern="${personCountPattern}" />
					</td>
					<td>
						<c:if test="${f:h(row.reserve.transfer) == 1}">
					...
					<td>
						<table class="com-button-table span-4">
							<tr>
								<td>
									<form:form method="get" action="${pageContext.request.contextPath}/reservations/${f:h(row.reserve.reserveNo)}">
										<button id="showBtn${rowStatus.index}">
											<spring:message code="label.tr.managereservation.showDetailsBtnMessage"/>
										</button>
									</form:form>
									<c:if test="${!f:h(row.limitExceeding)}">
										<c:if test="${f:h(row.reserve.transfer) == 0}">
											<form:form method="get" action="${pageContext.request.contextPath}/reservations/${f:h(row.reserve.reserveNo)}/update">
												<button id="changeBtn${rowStatus.index}" name="form">
													<spring:message code="label.tr.managereservation.changeReservationBtnMessage"/>
												</button>
											</form:form>
											<form:form method="get" action="${pageContext.request.contextPath}/reservations/${f:h(row.reserve.reserveNo)}/cancel">
